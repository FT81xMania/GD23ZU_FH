...
void IDEMCU()
{
     ...
     if (STM32_CPU==407){sprintf(Board, "MCU: STM32F407-M4DEMO"); MCUID=407;}
     if (STM32_CPU==429){sprintf(Board, "MCU: STM32 F49I-DISCO"); MCUID=429;}
     if (STM32_CPU==743){sprintf(Board, "MCU: STM32 Nucleo-H743ZI"); MCUID=743;}
     if (STM32_CPU==746){sprintf(Board, "MCU: STM32F746-Core7XXI"); MCUID=746;}
  ...
  #ifdef TEENSYDUINO
     ClaveBoard=2;
     //sprintf(Board, "MCU(%d): Teensy 3.X", ClaveBoard);
